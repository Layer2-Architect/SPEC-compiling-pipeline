...
AI レビュアは指摘を出した後、自分で降格してはいけない(`review-guidelines/severity.md` §2)。また、**品質基準そのものを緩める PR**(ハードルール変更、`.trace-engine.toml` chain 順変更、`scripts/trace-check.sh` のチェック削除など)は AI Approve 不可。人間 Approve 必須領域として `REQUEST_CHANGES` または `NEEDS_HUMAN` を返す。
...
- リリースゲート(AT 通過判定)
- `review-guidelines/` / `bootstrap/CLAUDE*.md.template` / `.trace-engine.toml` 等の品質基準変更

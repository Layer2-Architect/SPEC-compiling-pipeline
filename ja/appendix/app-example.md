...
├── .legixy.toml                     # area=APP, 13 typecode
...
│   ├── DevelopmentProcess/                # 採用前の旧プロセス文書（互換のため残置）
...
3. **Phase 2 → 3**: AT を独立 typecode として `.legixy.toml` に登録、リリース前ゲートに組み込み
...
| 種別 | パス | 説明 |
|---|---|---|
| プロセス本体（旧版）| `docs/DevelopmentProcess/`（旧プロセス文書） | フレームワーク化前の運用文書 |
| プロセス分割（旧版）| `docs/process/01-overview.md` 〜 `08-gates.md` | 同 |
| trace-engine 設定 | `.legixy.toml` | area=APP、13 typecode |
...
- APP プロジェクト本体（`docs/DevelopmentProcess/`、`docs/process/`、`.legixy.toml` 等）は APP の現行運用文書

#!/usr/bin/env bash
# trace-check.sh — SCP フレームワーク統合検証スクリプト
#
# 1. legixy check --formal （第 1 層: 形式検証）
# 2. legixy check          （第 1 層 + 第 2 層 semantic、ONNX モデル必須）
...
#
# Usage:
#   bash scripts/trace-check.sh                # 第 1 層 + 第 2 層（モデルあれば）+ ゲート
...
# ---------- [1/5] 第 1 層 ----------
echo "[1/5] legixy check --formal（第 1 層: 形式検証）"
if ! legixy check --formal; then
  fail_msg=$?
  red "  公式 check --formal が失敗しました（exit=$fail_msg）"
  FAIL=$((FAIL+1))
fi

# ---------- [2/5] 第 2 層 semantic ----------
echo ""
echo "[2/5] legixy check（第 2 層: semantic）"
...
if [[ $NO_SEMANTIC -eq 1 ]]; then
  ...
  if [[ $STRICT -eq 1 ]]; then
    ...
    yellow "  semantic 検証を有効化するには huggingface.co/sentence-transformers/all-MiniLM-L6-v2 から"
    yellow "  model.onnx と tokenizer.json を $(dirname "$ONNX_MODEL_PATH")/ に配置してください"
    WARN=$((WARN+1))
  fi
else
  if ! legixy check; then
    fail_msg=$?
...
# ---------- [3/5] SPEC レベル TDD ゲート ----------
echo ""
echo "[3/5] SPEC レベル TDD ループのゲート"
...
# ---------- [4/5] 前段ループゲート（ハードルール 9）----------
echo ""
echo "[4/5] 前段ループゲート（各 SPEC の最新 FCR が ACCEPTED か）"
...
if [[ ! -d "$SPEC_DIR" ]]; then
  ...
  if [[ -z "$SPEC_FILES" ]]; then
    ...
    while IFS= read -r spec_file; do
      # SPEC ID をファイル名から抽出（例: SPEC-APP-001_xxx.md → SPEC-APP-001）
...
# ---------- [5/5] レイヤ汚染検査（ハードルール 10）----------
echo ""
echo "[5/5] レイヤ汚染検査（具体側 RBD/SEQD に言語固有要素なし）"
